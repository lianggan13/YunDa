...
namespace Y.ASIS.Common.MVVMFoundation
{
    ...
    public class Messenger
    {
        ...
        /// <summary>
        /// UnRegisters a callback method by class name which it is
        /// </summary>
        /// <param name="message"></param>
        /// <param name="className"></param>

        public void UnRegister(string message, string className)
        {
            _messageToActionsMap.RemoveAction(message, className);
        }
        ...
        public void Clear()
        {
            _messageToActionsMap.ClearAction();
        }
        #endregion
        ...
        private class MessageToActionsMap
        {
            ...
            internal void AddAction(string message, object target, MethodInfo method, Type actionType)
            {
                ...
                lock (_map)
                {
                    ...
                    {
                        wa = _map[message].FirstOrDefault(a => a.Method.DeclaringType.FullName == method.DeclaringType.FullName);
                    }
                    else
                    {
                        wa = _map[message].FirstOrDefault(a => a.Method.DeclaringType.FullName == method.DeclaringType.FullName && a.ParameterType.FullName == actionType.FullName);
                    }

                    if (wa != null)
                    {
                        // _map[message].Remove(wa); // if the same action has already exist, remove first, and then add.
                    }
            ...
            internal void RemoveAction(string message, string className)
            {
                lock (_map)
                {
                    if (!_map.ContainsKey(message))
                    ...
                    var rmActions = weakActions.Where(a => a.Method.DeclaringType.FullName == className)?.ToList();
                    rmActions.ForEach(ra => weakActions.Remove(ra));
                }
            }
